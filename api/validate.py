...
def validate_register_user(body):
    ...
    if role == "librarian":
        ...
        if cursor.fetchone() is not None:
            return f"{user_id} is already registered as a member"
    ...
    if cursor.fetchone() is not None:
        return f"{email} already registered with an existing member"

    cursor.execute(f"SELECT * FROM {table} WHERE phone = %s", (phoneNo,))
    if cursor.fetchone() is not None:
        return f"{phoneNo} already registered with an existing member"
